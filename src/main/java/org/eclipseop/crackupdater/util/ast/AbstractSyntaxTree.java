package org.eclipseop.crackupdater.util.ast;

import org.eclipseop.crackupdater.util.ast.expression.Expression;
import org.eclipseop.crackupdater.util.ast.expression.impl.*;
...
/** Created by Eclipseop. Date: 7/27/2017. */
// TODO: 6/11/2022 rewrite all of this :3
public class AbstractSyntaxTree {
    ...
    Arrays.stream(methodNode.instructions.toArray())
        .forEach(
            ain -> {
                ...
                case Opcodes.IFNONNULL:
                  ...
                  stack.add(
                      new ConditionExpression(
                          ain, lastExpression, new VarExpression(null, "null")));
                  break;
                case Opcodes.IFNE:
                ...
                case Opcodes.IF_ICMPGE:
                  ...
                  */
                  ...
                  if (opcode == Opcodes.IFNE) {
                    stack.add(
                        new ConditionExpression(ain, lastExpression, new IntegerExpression(null)));
                    break;
                  }

                  if (stack.size() >= 2) {
                    stack.add(
                        new ConditionExpression(ain, stack.get(stack.size() - 2), lastExpression));
                  }
                  break;
              ...
              if ((opcodesToFind.length == 0 && stack.get(stack.size() - 1).consumesStack())
                  || IntStream.of(opcodesToFind).anyMatch(p -> p == opcode)) {
  ...
  public static List<Expression> find(final List<ClassNode> list, final int opcodeToFind) {
    ...
    list.stream()
        ...
        .filter(p -> !p.name.equals("<init>") && !p.name.equals("<cinit>"))
        .forEach(
            c -> {
              List<Expression> expressions = find(c, opcodeToFind);
